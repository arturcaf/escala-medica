...
/**
 * Escala Medica v6 - CallMeBot WhatsApp + SQLite + SSE
 *
 * CallMeBot: API gratuita para enviar mensagens WhatsApp
 * Cada medico ativa individualmente enviando mensagem para +34 644 59 21 99
 * Documentacao: https://www.callmebot.com/blog/free-api-whatsapp-messages/
 *
 * Notificacoes automaticas:
 * - Troca APROVADA -> avisa solicitante e substituto
 * - Troca RECUSADA -> avisa solicitante com motivo
 * - Admin edita plantao -> avisa medico afetado
 * - Plantao A DEFINIR -> lembrete diario as 8h
 */
public class EscalaServer {
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("\n  ╔══════════════════════════════════════════╗");
        System.out.println("  ║   ESCALA MEDICA v6 - WhatsApp + SQLite   ║");
        System.out.println("  ║   CallMeBot | SSE | Login | GPS          ║");
        System.out.println("  ╚══════════════════════════════════════════╝\n");
        ...
        System.out.println("  WhatsApp: CallMeBot (gratuito)");
        ...
        System.out.println("  WhatsApp: /api/whatsapp\n");

        // Schedule daily reminder for A DEFINIR plantoes at 8am
        scheduleReminders();
    }

    // ── CALLMEBOT WHATSAPP ───────────────────────────────────
    /**
     * Envia mensagem WhatsApp via CallMeBot (gratuito)
     * Cada medico precisa ativar enviando:
     * "I allow callmebot to send me messages" para +34 644 59 21 99
     * e recebe sua apikey pessoal
     */
    static void sendWhatsApp(String phone, String apiKey, String message) {
        if (phone == null || phone.isEmpty() || apiKey == null || apiKey.isEmpty()) {
            System.out.println("  WhatsApp: telefone ou apikey nao configurado para " + phone);
            return;
        }
        ...
        new Thread(() -> {
            try {
                ...
                System.out.println("  WhatsApp enviado para " + cleanPhone + " | HTTP: " + code);
                conn.disconnect();
            } catch (Exception e) {
                System.err.println("  WhatsApp ERRO para " + phone + ": " + e.getMessage());
            }
        }).start();
    }

    /**
     * Busca apikey do medico no banco de dados
     */
    static String getApiKey(String nomeMedico) {
        try {
            PreparedStatement ps = db.prepareStatement("SELECT apikey FROM whatsapp_keys WHERE medico=?");
            ps.setString(1, nomeMedico);
            ResultSet rs = ps.executeQuery();
            String key = rs.next() ? rs.getString("apikey") : null;
            rs.close(); ps.close();
            return key;
    ...
    /**
     * Busca telefone do medico pelo nome
     */
    static String getTelefone(String nomeMedico) {
        return medicos.stream()
            .filter(m -> m[1].equals(nomeMedico))
            .map(m -> m[2])
            .findFirst().orElse(null);
    }

    /**
     * Envia WhatsApp para um medico pelo nome
     */
    static void notificarMedico(String nomeMedico, String mensagem) {
        String tel = getTelefone(nomeMedico);
        String key = getApiKey(nomeMedico);
        if (tel != null && key != null) {
            sendWhatsApp(tel, key, mensagem);
        }
    }

    /**
     * Agenda lembretes diarios para plantoes A DEFINIR
     */
    static void scheduleReminders() {
        ...
        scheduler.scheduleAtFixedRate(() -> {
            try {
                ...
                List<String[]> aDefinir = escala.stream()
                    .filter(r -> "A DEFINIR".equals(r[6]) && r[1].compareTo(hoje) >= 0 && r[1].compareTo(cutoff) <= 0)
                    .collect(Collectors.toList());

                if (!aDefinir.isEmpty()) {
                    String msg = "🏥 ESCALA MEDICA - ALERTA\n" +
                        "Plantoes SEM plantonista nos proximos 7 dias:\n";
                    for (String[] p : aDefinir) {
                        msg += "• " + p[1] + " (" + p[2] + ") - " + p[3] + "\n";
                    }
                    msg += "\nAcesse o app para resolver.";
                    // Notify admin
                    notificarMedico(ADMIN_NOME, msg);
                    System.out.println("  Lembrete enviado: " + aDefinir.size() + " plantoes A DEFINIR");
                }
            } catch (Exception e) {
                System.err.println("Erro no lembrete: " + e.getMessage());
            }
        }, delay, 24 * 60, java.util.concurrent.TimeUnit.MINUTES);

        System.out.println("  Lembretes agendados para 08:00 diariamente");
    }
    ...
    static void handleSSE(HttpExchange ex) throws IOException {
        ...
        sseClients.add(ex);
        try {
            ex.getResponseBody().write("data: {\"type\":\"connected\"}\n\n".getBytes(StandardCharsets.UTF_8));
    ...
    static void broadcast(String eventType, String data) {
        ...
        synchronized (sseClients) {
            for (HttpExchange client : sseClients) {
                try { client.getResponseBody().write(bytes); client.getResponseBody().flush(); }
    ...
    static void addNotificacaoAndBroadcast(String tipo, String titulo, String mensagem) {
        ...
        String notifJson = "{\"tipo\":\"" + escapeJson(tipo) + "\",\"titulo\":\"" + escapeJson(titulo) + "\",\"mensagem\":\"" + escapeJson(mensagem) + "\"}";
    ...
    static void handleLogin(HttpExchange ex) throws IOException {
        ...
        if (medico != null) {
            ...
            if (tel.endsWith(senhaLimpa) || senhaLimpa.equals(tel)) {
                ...
                String apiKey = getApiKey(medico[1]);
                boolean whatsappAtivo = apiKey != null && !apiKey.isEmpty();
                sendJson(ex,200,"{\"token\":\""+token+"\",\"nome\":\""+escapeJson(medico[1])+"\",\"crm\":\""+medico[0]+"\",\"isAdmin\":false,\"notificacoesPendentes\":"+pend+",\"whatsappAtivo\":"+whatsappAtivo+"}");
                return;
            }
        }
    ...
    static void handleMe(HttpExchange ex) throws IOException {
        ...
        boolean whatsappAtivo = apiKey != null && !apiKey.isEmpty();
        sendJson(ex,200,"{\"nome\":\""+escapeJson(sess.get("nome"))+"\",\"crm\":\""+sess.get("crm")+"\",\"isAdmin\":"+sess.get("isAdmin")+",\"notificacoesPendentes\":"+pend+",\"whatsappAtivo\":"+whatsappAtivo+",\"sseClients\":"+sseClients.size()+"}");
    }

    // GET/PUT /api/whatsapp - manage WhatsApp keys
        ...
        try {
            if ("GET".equals(method)) {
                ...
                while (rs.next()) {
                    ...
                    k.put("medico", rs.getString("medico"));
                    k.put("telefone", rs.getString("telefone"));
                    k.put("apikey", rs.getString("apikey") != null ? "****" + rs.getString("apikey").substring(Math.max(0,rs.getString("apikey").length()-4)) : "");
                    k.put("ativo", String.valueOf(rs.getInt("ativo")));
                    k.put("createdAt", rs.getString("created_at"));
                    result.add(k);
                }
                rs.close();
                ...
                // Register/update WhatsApp key for a medico
                Map<String,String> sess = getSession(ex);
                if (sess == null) { sendJson(ex,401,"{\"error\":\"Nao autenticado\"}"); return; }
                Map<String,String> req = parseJsonObject(new String(ex.getRequestBody().readAllBytes(), StandardCharsets.UTF_8));
                String apiKey = req.get("apikey");
                String medico = req.getOrDefault("medico", sess.get("nome"));
                // Only admin can register for others
                if (!sess.get("nome").equals(medico) && !Boolean.parseBoolean(sess.get("isAdmin"))) {
                    sendJson(ex,403,"{\"error\":\"Sem permissao\"}"); return;
                }
                if (apiKey == null || apiKey.trim().isEmpty()) {
                    sendJson(ex,400,"{\"error\":\"apikey obrigatoria\"}"); return;
                }
                ...
                ps.setString(1, medico); ps.setString(2, tel != null ? tel : "");
                ps.setString(3, apiKey.trim()); ps.setString(4, now());
                ps.executeUpdate(); ps.close();
                ...
                Map<String,String> sess = getSession(ex);
                if (sess == null) { sendJson(ex,401,"{\"error\":\"Nao autenticado\"}"); return; }
                String medico = sess.get("nome");
                db.prepareStatement("DELETE FROM whatsapp_keys WHERE medico='" + medico + "'").executeUpdate();
                sendJson(ex,200,"{\"ok\":true}");

            } else { send405(ex); }
        } catch (Exception e) { sendJson(ex,500,"{\"error\":\""+escapeJson(e.getMessage())+"\"}"); }
    }
    ...
    static void handleTrocas(HttpExchange ex) throws IOException {
        ...
        try {
            if("GET".equals(method)){
                ...
                String waAdmin = "🔄 NOVA SOLICITACAO DE TROCA\nID: " + id + "\n📅 " + plantao[1] + " (" + plantao[2] + ")\n⏰ " + plantao[3] + "\n👤 Solicitante: " + sol + "\n👤 Substituto: " + sub + "\n💬 Motivo: " + mot + "\n\nAcesse o app para aprovar ou recusar.";
                notificarMedico(ADMIN_NOME, waAdmin);
                ...
                String pid=rs.getString("plantao_id"),sub=rs.getString("substituto"),sol=rs.getString("solicitante"),dp=rs.getString("data_plantao"),tur=rs.getString("turno"),dia=rs.getString("dia_plantao");
                rs.close();
                String nowStr=now();
                if("aprovar".equals(acao)){
                    ...
                    String waSol = "✅ TROCA APROVADA - " + id + "\n📅 " + dp + " (" + dia + ")\n⏰ " + tur + "\n\nSua solicitacao de troca foi APROVADA pelo Dr. Artur Cesar.\n" + (og.isEmpty() ? "" : "Obs: " + og) + "\n\nVoce esta DISPENSADO deste plantao.";
                    notificarMedico(sol, waSol);

                    String waSub = "✅ TROCA APROVADA - " + id + "\n📅 " + dp + " (" + dia + ")\n⏰ " + tur + "\n\nVoce foi CONFIRMADO como plantonista nesta data.\nSolicitante original: " + sol + "\n\nConfirme sua presenca no app.";
                    notificarMedico(sub, waSub);
                    ...
                    // WhatsApp: notify solicitante
                    String waRec = "❌ TROCA RECUSADA - " + id + "\n📅 " + dp + " (" + dia + ")\n⏰ " + tur + "\n\nSua solicitacao de troca foi RECUSADA pelo Dr. Artur Cesar.\nMotivo: " + obsGestor + "\n\nVoce permanece escalado neste plantao.";
                    notificarMedico(sol, waRec);
